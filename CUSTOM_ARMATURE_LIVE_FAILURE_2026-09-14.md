...
The first runtime 1.7 launch ended before the validation action. Its last telemetry remained `DISABLED` with intent revision zero and all discovery, pose, plan, and publication counters at zero. Windows reported `D3DDRED2` error `0x887A0006`, `LiveKernelEvent 141`, and then a `BEX64` termination in `ntdll.dll`. Because F8 was never accepted and the custom worker's scan/service branch never ran, this launch neither validates nor disproves the runtime 1.7 frame-handoff correction. No automatic relaunch was performed.
